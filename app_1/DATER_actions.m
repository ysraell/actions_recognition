%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Program to actions recognition by DATER (Li and Schonfeld, 2014).
...
function [R_DATER_actions,Cl_DATER_actions,Dist_DATER_actions] = DATER_actions(classe_teste,classe_treino,sinalTeste,sinalTreino,Tmax,tol,Dim_red)
    ...
    while (t<=Tmax)&&(err>tol)
        ...
        err = max([norm(Ul-Ulb) norm(Uc-Ucb)]);
        disp([t err])
...
%                     dist_mii(Mii) = norm(sinalTeste_proj{Ni}{Mi}-sinalTreino_proj{Nii}{Mii});
    ...
    R_DATER_actions =  sum(Cl)/N;
    Cl_DATER_actions = Cl;
    Dist_DATER_actions = dist_ni;
...
%     end
%     
%     R_DATER_actions = R_Cl;
%     Cl_DATER_actions = 0;
%     Dist_DATER_actions = 0;
% end